...
def alerts():
    ...
    fig = go.Figure(data=[go.Scatter(x=alerts['Date'],y=alerts['Alerts'])],
                    layout_title_text='ALERTS DAILY',layout=go.Layout(height=800))
    import plotly
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON = json.dumps(fig, cls=a)
    return render_template('home.html',graphJSON=graphJSON)
...
def categories():
    ...
    fig = go.Figure(data=[go.Bar(y=alerts_number,x=alerts_types)],
                    layout_title_text='ALERTS BY CATEGORIES',layout=go.Layout(height=800))
    import plotly
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON = json.dumps(fig, cls=a)
    return render_template('home.html',graphJSON=graphJSON)
...
def puertos():
    ...
    fig = go.Figure(data=[go.Table(header=dict(values=['MEDIA PUERTOS',"TOTAL SERVICIOS INSEGUROS","TOTAL SERVICIOS"]),
                                    cells= dict(values=[[media_puertos],[servicios_inseguros],[servicios]]))],
                                    layout_title_text='PORTS METRICS',layout=go.Layout(height=800))
    import plotly
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON = json.dumps(fig, cls=a)
    return render_template('home.html',graphJSON=graphJSON)
